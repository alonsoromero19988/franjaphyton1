# Tipos de datos

# Tipos de datos clásicos
# 1.- Números enteros
# 2.- Números decimales
# 3.- Cadenas de texto (string)
# 4.- Booleanos

# 2.- Números decimales
# numero_decimal = 3.5
# print(numero_decimal)

# 3.- Cadenas de texto (string)
#nombre = "Juan"
#print(nombre)

# nombre2 = 'María José'
# print(nombre2)

# nombre3 = """ Este es mi texto
# En varias lineas
# """
# print(nombre3)

# # Concatenación
# nombre4 = nombre + nombre2
# print(nombre4)

# print(nombre * 3)

# print(nombre + ", " + nombre2)



# 4.- Booleanos

# es_estudiante = True
# print(es_estudiante)

# trabaja = False
# print(trabaja)



# Convertir tipos de datos

# Convertir texto a numeros
numero1 = input("Ingrese un número:")
print(numero1)

numero2 = input("Ingrese otro número: ")
print(numero2)

print(numero1 + numero2) ### siempre reconocera un texto en cadena

numero1 = int(numero1)
numero2 = int(numero2)
print(numero1 + numero2) 


numero_decimal = 4.5
print(int(numero_decimal)) #int: numero entero 

# "4.5"
print(str(numero_decimal))  #srt: transforma a texto